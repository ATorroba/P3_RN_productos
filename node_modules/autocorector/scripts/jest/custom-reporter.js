...
onRunComplete(test, results) {
  try{
    Utils.debug('Terminado de ejecutar el test-suite');
    const {
      numFailedTests,
      numPassedTests,
      numTodoTests,
    ...
    testResults.map(({failureMessage}) => {
        if (failureMessage) {
            Utils.debug("MENSAJE DE FALLO DE JEST:");
            Utils.debug(failureMessage);
        }
    });
...
onTestCaseResult(test, testCaseResult) {
  try{
    ...
    //Utils.debug("TEST RESULT: ", testCaseResult);
...
onTestResult(test, testResult) {
  //Utils.debug("TEST RESULT: ", testResult);
